...
torch.save(loss, "64loss.pt")

# for x, y, x_length, y_length in dev_data:
#     x = x.to(DEVICE)
#     y = y.to(DEVICE)
#
#     for i in range(x.shape[0]):
#         predict, enc_self_attns, dec_self_attns, dec_enc_attns = model(x[i], y[i])
#         predict = predict.data.max(1, keepdim=True)[1]
#         print(x[i], '->', [zh_itow[n.item()] for n in predict.squeeze()])
#         print(x[i], '->', [zh_itow[n] for n in x[i]])
