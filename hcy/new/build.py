...
def build_vocab(args):
    # 用训练集构建字典
    en_iter = FileLister([args.test_en_file])
    en_iter = FileOpener(en_iter).readlines().map(lambda x: x[1])

    cn_iter = FileLister([args.test_cn_file])
    cn_iter = FileOpener(cn_iter).readlines().map(lambda x: x[1])

    start_time = time.time()
    tokenizer_en = get_tokenizer('basic_english')
    tokenizer_cn = get_tokenizer(jieba.lcut)  # 结巴分词
    if False: # os.path.exists("en-zh/vocab_en.pt"):
        vocab_en = torch.load("en-zh/vocab_en.pt")
    else:
        # 字典中插入
        vocab_en = build_vocab_from_iterator(map(tokenizer_en, en_iter), min_freq=2,  # max_tokens = args.vocab_size,
    ...
    if False: # os.path.exists("en-zh/vocab_cn.pt"):
        vocab_cn = torch.load("en-zh/vocab_cn.pt")
    else:
        vocab_cn = build_vocab_from_iterator(map(tokenizer_cn, cn_iter), min_freq=2,  # max_tokens = args.vocab_size,
                                             specials = ['<unk>', '<pad>', '<bos>', '<eos>'])
